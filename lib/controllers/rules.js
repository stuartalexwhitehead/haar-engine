...
router.get('/rules/:rule', (req, res) => {
  RuleModel.findById(req.params.rule, (err, rule) => {
    if (err) {
      return res.json(resHelpers.error('The rule could not be found.'));
    }

    if (rule === null) {
      return res.json(resHelpers.fail('The rule could not be found.'));
    }
